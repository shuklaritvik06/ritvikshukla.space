...
function createTagCount(allBlogs) {
  ...
  allBlogs.forEach((file) => {
    if (file.type === 'Blog' && file.tags && file.draft !== true) {
...
function createSearchIndex(allBlogs) {
  if (
    ...
    const blogPosts = allBlogs.filter((doc) => doc.type === 'Blog' && doc.draft !== true)
    writeFileSync(
      `public/${siteMetadata.search.kbarConfig.searchDocumentsPath}`,
      JSON.stringify(allCoreContent(sortPosts(blogPosts)))
    )
  }
}
